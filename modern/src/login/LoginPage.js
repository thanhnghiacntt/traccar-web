...
import {
  useMediaQuery, InputLabel, Select, MenuItem, FormControl, Alert, Button, TextField, Link, Snackbar, IconButton, Tooltip,
} from '@mui/material';
  ...
  const handlePasswordLogin = async (event) => {
    ...
    try {
      ...
      if (error.message === '400') {
        const expirationMessage = announcements?.filter((it) => it.type === 3)[0];
    ...
    <LoginLayout>
      ...
      <div className={classes.container}>
        ...
        <TextField
          ...
          helperText={failed && t('invalidUser')}
        />
        <TextField
          required
          error={failed}
        ...
        <Stack direction="row" alignItems="center" gap={1}>
          ...
          <Typography variant="body2" style={{ fontWeight: 500 }}>
            {t('userSupport')}
            &nbsp;
            <Link href="tel:0901732888">090 173 2888</Link>
          </Typography>
        </Stack>
      </div>
      {expirationMessage && (
