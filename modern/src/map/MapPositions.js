...
const MapPositions = ({ positions, onClick, showStatus, selectedPosition, titleField }) => {
  ...
  const createFeature = (devices, position, selectedPositionId) => {
    ...
    const diff = moment.duration(moment(now).diff(moment(position.fixTime)));
    if (device.status === 'online' && position.speed > 0 && diff.asMinutes() < 10) { pulse = true; }
  ...
  }, [mapCluster, pulse, clusters, direction, onMarkerClick, onClusterClick]);
