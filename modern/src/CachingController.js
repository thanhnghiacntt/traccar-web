...
import {
  geofencesActions, tollstationsActions, groupsActions, driversActions, maintenancesActions, calendarsActions,
} from './store';
...
const CachingController = () => {
  ...
  useEffectAsync(async () => {
    if (authenticated) {
      ...
      if (response.ok) {
        dispatch(tollstationsActions.update(await response.json()));
      } else {
        throw Error(await response.text());
      }
    }
