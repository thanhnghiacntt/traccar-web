...
const CameraLiveCard = ({ deviceId, position, disableActions, desktopPadding = 0 }) => {
  ...
  // console.log(`showLiveCameraId${showLiveCameraId}`);
  ...
  const camID = device ? device.imeimdvr : 0;
  ...
  useEffectAsync(async () => {
    ...
    if (response.ok) {
      ...
      if (swfobject === null) {
        ...
        swfobject.setVideoInfo(0, `${device.name} CH1`);
        swfobject.setVideoInfo(1, `${device.name} CH2`);
        swfobject.setVideoInfo(2, `${device.name} CH3`);
        swfobject.setVideoInfo(3, `${device.name} CH4`);
        ...
        // dispatch(devicesActions.selectSwf(swfobject));
      }
    } else {
      throw Error(await response.text());
    }
