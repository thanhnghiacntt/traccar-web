...
const ExceedingSpeedLimit = ({ keyTitle }) => {
  ...
  const resultsTable = (data) => (
    <Table>
      <TableHead>
        <TableRow key="0">
          {[t('order'), t('time'), t('sharedMaintenance'), t('sharedGeofence')].map((text) => (
  ...
  const handleSearch = () => {
    ...
    fetch(`/api/reports/events?type=deviceOverspeed&from=${fromDateTime}&to=${toDateTime}&deviceId=${vehicleId}`, {
      method: 'GET',
      headers: {
