...
const DriverSummaryReport = ({ keyTitle }) => {
  ...
  const list = ['order', 'startTime', 'endTime', 'startOdometer', 'endOdometer', 'averageSpeed', 'maxSpeed', 'engineHours', 'spentFuel'];
      ...
      <TableBody>
        {data.map((row, index) => (
          <TableRow className={classes.rowHover} key={`${index + 1}`}>
            ...
            <TableCell align="center" className={classes.cell}>{formatHours(row.maxSpeed)}</TableCell>
