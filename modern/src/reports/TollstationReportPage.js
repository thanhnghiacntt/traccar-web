...
const columnsArray = [
  ...
  ['tollstationId', 'sysTollstation'],
  ['tollstationfee', 'sysTollstationfee'],
];
const columnsMap = new Map(columnsArray);

const TollstationReportPage = () => {
  ...
  const [columns, setColumns] = usePersistedState('eventColumns', ['eventTime', 'type', 'tollstationId', 'tollstationfee']);
  const [eventTypes, setEventTypes] = useState(['allEvents']);
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null);
  const [position, setPosition] = useState(null);
  const MoneyFormat = (value) => new Intl.NumberFormat('vi-VN', {
    style: 'currency',
    currency: 'VND',
  }).format(value);
  ...
  useEffectAsync(async () => {
    const response = await fetch('/api/notifications/tolltypes');
    if (response.ok) {
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (eventTypes[0] === 'allEvents') {
      ['tollstationEnter', 'tollstationExit'].forEach((it) => query.append('type', it));
    } else {
  ...
  const formatValue = (item, key) => {
    switch (key) {
      ...
      case 'tollstationId':
        if (item[key] > 0) {
          const tollstation = tollstations[item[key]];
          return tollstation && tollstation.name;
        }
        return null;
      case 'tollstationfee':
        if (item.type === 'tollstationExit') {
          if (item.attributes.tollprice !== undefined) { return MoneyFormat(item.attributes.tollprice); }
        }
        return '';
      case 'maintenanceId':
      ...
      case 'attributes':
        switch (item.type) {
          ...
          case 'tollstationExit':
            return item.attributes.tollprice;
          default:
            return '';
        }
      default:
...
export default TollstationReportPage;
