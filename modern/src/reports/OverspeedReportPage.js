...
const OverspeedReportPage = () => {
  ...
  const [columns, setColumns] = usePersistedState('eventColumns', ['eventTime', 'type']);
  const [eventTypes, setEventTypes] = useState(['deviceOverspeed']);
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (eventTypes[0] === 'allEvents') {
      ['deviceOverspeed'].forEach((it) => query.append('type', it));
    } else {
