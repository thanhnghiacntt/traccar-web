...
import {
  formatDistance, formatHours, formatSpeed, formatVolume, formatTime, formatMoney,
...
const columnsArray = [
  ...
  ['fuelPrice', 'fuelPrice'],
  ['fuelType', 'fuelType'],
  ['spentFuelByDistance', 'spentFuelByDistance'],
  ['spentPrice', 'spentPrice'],

];
const columnsMap = new Map(columnsArray);

const FuelSummaryReportPage = () => {
  ...
  const [columns, setColumns] = usePersistedState('summaryColumns', ['startTime', 'distance', 'fuelByKM', 'spentFuelByDistance', 'fuelType', 'fuelPrice', 'spentPrice']);
  ...
  const handleSubmit = useCatch(async ({ deviceIds, groupIds, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/api/reports/fuelsummary/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/api/reports/fuelsummary/mail?${query.toString()}`);
      if (!response.ok) {
      ...
      setLoading(true);
      try {
        const response = await fetch(`/api/reports/fuelsummary?${query.toString()}`, {
...
export default FuelSummaryReportPage;
